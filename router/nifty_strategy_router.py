...
# Create router
nifty_strategy_router = APIRouter(
    prefix="/api/v1/nifty-strategy", tags=["NIFTY Strategy"]
)
...
class NiftyStrategyConfigUpdate(BaseModel):
    ...
    ema_period: Optional[int] = Field(
        None, ge=5, le=50, description="EMA period (5-50)"
    )
    strength_threshold: Optional[float] = Field(
        None, ge=0.1, le=1.0, description="Candle strength threshold (0.1-1.0)"
    )
    volume_multiplier: Optional[float] = Field(
        None, ge=0.5, le=5.0, description="Volume multiplier (0.5-5.0)"
    )
    stop_loss_pct: Optional[float] = Field(
        None, ge=0.5, le=10.0, description="Stop loss percentage (0.5-10.0)"
    )
    target_pct: Optional[float] = Field(
        None, ge=1.0, le=20.0, description="Target percentage (1.0-20.0)"
    )
    position_size: Optional[float] = Field(
        None, gt=0, description="Position size in rupees"
    )
    max_daily_trades: Optional[int] = Field(
        None, ge=1, le=10, description="Maximum daily trades (1-10)"
    )
...
async def start_nifty_strategy():
    ...
    try:
        ...
        if result:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to start NIFTY strategy"
            )
...
async def update_nifty_strategy_config(config_update: NiftyStrategyConfigUpdate):
    ...
    try:
        ...
        return NiftyStrategyResponse(
            ...
            data={"updated_config": update_dict},
        )
...
async def get_nifty_live_data():
    ...
    try:
        ...
        buffer_data = (
            strategy.ohlcv_buffer.tail(10).to_dict("records")
            if not strategy.ohlcv_buffer.empty
            else []
        )
