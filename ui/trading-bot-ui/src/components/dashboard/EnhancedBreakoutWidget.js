...
const EnhancedBreakoutWidget = ({ 
  ...
  const processedData = useMemo(() => {
    ...
    const uniqueBreakouts = allBreakouts.reduce((acc, current) => {
      ...
      if (!acc.some(item => `${item.instrument_key}_${item.timestamp}` === key)) {
        acc.push(current);
      }
      return acc;
    }, []);
    ...
    const filteredBreakouts = uniqueBreakouts.filter(breakout => {
      // Type filter
      if (selectedType !== "all") {
        if (selectedType === "bullish" && !["volume_breakout", "momentum_breakout", "resistance_breakout", "gap_up"].includes(breakout.breakout_type)) {
          return false;
        }
        if (selectedType === "bearish" && !["support_breakdown", "gap_down", "low_breakdown"].includes(breakout.breakout_type)) {
          return false;
        }
        if (selectedType !== "bullish" && selectedType !== "bearish" && breakout.breakout_type !== selectedType) {
          return false;
        }
      }
    ...
    filteredBreakouts.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
  ...
  if (isLoading) {
    return (
      <Paper sx={{ backgroundColor: bloombergColors.cardBg, p: 2, height: "100%" }}>
        <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
          <Typography variant="h6" sx={{ color: bloombergColors.primary, mb: 2 }}>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <Box sx={{ display: "flex", gap: 1, mb: 2, flexWrap: "wrap" }}>
          <FormControl size="small" sx={{ minWidth: 120 }}>
            <InputLabel sx={{ color: bloombergColors.textSecondary }}>Type</InputLabel>
            <Select
              value={selectedType}
          ...
          <FormControl size="small" sx={{ minWidth: 120 }}>
            <InputLabel sx={{ color: bloombergColors.textSecondary }}>Strength</InputLabel>
            <Select
        ...
        {showMetrics && metrics && (
          <Paper sx={{ p: 1, mb: 2, backgroundColor: bloombergColors.surfaceVariant }}>
            <Grid2 container spacing={1}>
              <Grid2 xs={6}>
                <Box sx={{ textAlign: "center" }}>
                  <Typography variant="caption" sx={{ color: bloombergColors.textSecondary }}>
                    Instruments
                  </Typography>
                  <Typography variant="h6" sx={{ color: bloombergColors.primary }}>
              ...
              <Grid2 xs={6}>
                <Box sx={{ textAlign: "center" }}>
                  ...
                  <Typography variant="h6" sx={{ color: bloombergColors.success }}>
              ...
              <Grid2 xs={6}>
                <Box sx={{ textAlign: "center" }}>
                  ...
                  <Typography variant="h6" sx={{ color: bloombergColors.warning }}>
        ...
        <Box sx={{ flexGrow: 1, overflow: "auto" }}>
          {breakouts.length === 0 ? (
            <Box sx={{ 
              ...
              <ShowChart sx={{ color: bloombergColors.textSecondary, fontSize: 48, mb: 1 }} />
              <Typography sx={{ color: bloombergColors.textSecondary, textAlign: "center" }}>
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 0.5 }}>
                          ...
                          <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                            ...
                            <Typography 
                              variant="body2" 
                              sx={{ 
                                color: isPositive ? bloombergColors.success : bloombergColors.error,
                                fontWeight: "bold"
                              }}
                            >
                              {isPositive ? "+" : ""}{breakout.percentage_move?.toFixed(2)}%
                            </Typography>
                          </Box>
                        </Box>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                          ...
                          {breakout.volume_ratio && breakout.volume_ratio > 1 && (
                            <Chip
                              label={`${breakout.volume_ratio.toFixed(1)}x Vol`}
                              size="small"
                              sx={{
                          ...
                          {breakout.confirmation_signals && breakout.confirmation_signals.length > 0 && (
                            <Tooltip title={breakout.confirmation_signals.join(", ")}>
                              <Chip
                                label="Confirmed"
                                size="small"
                                sx={{
                                  backgroundColor: bloombergColors.success + "20",
                      ...
                      <Box sx={{ textAlign: "right" }}>
                        ...
                        {breakout.confidence && (
                          <Typography variant="caption" sx={{ color: bloombergColors.textSecondary, display: "block" }}>
