...
#                 self.risk_per_trade = (
#                     user_config.stop_loss_percent or self.risk_per_trade
#                 )
#                 self.max_concurrent_trades = (
#                     user_config.max_positions or self.max_concurrent_trades
#                 )
#                 logger.info(
#                     f"✅ Loaded user config: qty={self.default_quantity}, risk={self.risk_per_trade}%"
#                 )
...
#     async def _get_selected_stocks(self) -> List[Any]:
#         """Get today's selected stocks"""
#         try:
#             db = SessionLocal()
...
#                 except Exception as e:
#                     logger.error(
#                         f"❌ Error processing selected stock {record.symbol}: {e}"
#                     )
#                     continue
...
#     async def _register_price_callbacks(
#         self, selected_stocks: List[StockSelectionResult]
#     ):
#         """Register for live price updates"""
#         try:
#             instrument_keys = []
...
#                 if stock.option_contract and stock.option_contract.get(
#                     "instrument_key"
#                 ):
...
#                 "auto_trade_execution", instrument_keys, self._handle_price_update
#             )
...
#             asyncio.create_task(
#                 self._update_trade_positions(instrument_key, price_data)
#             )
...
#                     logger.info(
#                         f"📈 Generated {signal.direction.value} signal for {signal.symbol} {signal.option_type}"
#                     )
...
#             stop_loss, target = self._calculate_stop_loss_target(
#                 option_ltp, option_type
#             )
...
#     def _calculate_stop_loss_target(
#         self, option_price: float, option_type: str
#     ) -> Tuple[float, float]:
...
#                 logger.warning(
#                     f"⚠️ Maximum concurrent trades reached ({self.max_concurrent_trades})"
#                 )
#                 return
...
#                 logger.info(
#                     f"📄 PAPER TRADE: {signal.symbol} {signal.option_type} @ ₹{signal.trigger_price}"
#                 )
#             else:
...
#                     trade_order.entry_price = broker_result.get(
#                         "execution_price", signal.trigger_price
#                     )
#                     logger.info(
#                         f"🔴 LIVE TRADE: {signal.symbol} {signal.option_type} @ ₹{trade_order.entry_price}"
#                     )
#                 else:
#                     trade_order.status = OrderStatus.REJECTED
#                     logger.error(
#                         f"❌ LIVE TRADE FAILED: {broker_result.get('error', 'Unknown error')}"
#                     )
#                     return
...
#         self, signal: TradeSignal, trade_order: TradeOrder
#     ):
...
#             # Stop loss check
#             if (
#                 trade_order.direction == TradeDirection.BUY
#                 and current_price <= trade_order.stop_loss
#             ):
#                 should_exit = True
...
#             if (
#                 trade_order.direction == TradeDirection.BUY
#                 and current_price >= trade_order.target
#             ):
#                 should_exit = True
#                 exit_reason = "TARGET"
...
#         self, trade_id: str, trade_order: TradeOrder, exit_reason: str
#     ):
#         """Exit a trade"""
#         try:
...
#         self, trade_order: TradeOrder, signal: TradeSignal
#     ) -> Dict[str, Any]:
...
#                 .filter(
#                     BrokerConfig.user_id == self.user_id, BrokerConfig.is_active == True
#                 )
#                 .first()
#             )
#             db.close()
...
#             if result["success"]:
#                 logger.info(
#                     f"🔴 LIVE ORDER PLACED: {signal.symbol} {signal.option_type} via {broker_name.upper()}"
#                 )
#                 return {
#                     "success": True,
#                     "order_id": result.get("order_id"),
#                     "execution_price": result.get(
#                         "execution_price", signal.trigger_price
#                     ),
...
#             else:
#                 logger.error(
#                     f"❌ LIVE ORDER FAILED: {result.get('error', 'Unknown broker error')}"
#                 )
#                 return {
#                     "success": False,
