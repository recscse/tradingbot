...
class TokenMonitorService:
    ...
    async def monitor_all_tokens(self) -> Dict[str, int]:
        ...
        try:
            ...
            active_configs = (
                ...
                .filter(
                    and_(
                        ...
                        BrokerConfig.access_token_expiry.isnot(None),
                    )
                )
                .all()
            )
            ...
            logger.info(
                f"🔍 Monitoring {len(active_configs)} active broker configurations"
            )
            ...
            for config in active_configs:
                try:
                    ...
                    logger.error(
                        f"❌ Error checking token for {config.broker_name}: {e}"
                    )
                    results["errors"] += 1
    ...
    async def check_token_expiry(
        self, config: BrokerConfig, db: Session
    ) -> Optional[str]:
        """
        ...
        """
        if not config.access_token_expiry:
            return None

        now = datetime.utcnow()
        time_until_expiry = config.access_token_expiry - now
        ...
        try:
            ...
            logger.error(
                f"❌ Error processing token expiry for {config.broker_name}: {e}"
            )
    ...
    async def handle_expired_token(self, config: BrokerConfig, db: Session):
        ...
        try:
            ...
            config.last_error = (
                f"Token expired at {config.access_token_expiry.isoformat()}"
            )
            ...
            await notification_service.send_multi_channel_notification(
                ...
                metadata={
                    "broker_name": config.broker_name,
                    "broker_id": config.id,
                    "expired_at": config.access_token_expiry.isoformat(),
                    "auto_disabled": True,
                },
                db=db,
            )
            ...
            user_expired_count = (
                ...
                .filter(
                    and_(
                        ...
                        BrokerConfig.access_token_expiry < datetime.utcnow(),
                    )
                )
                .count()
            )
    ...
    def mark_notification_sent(self, key: str, priority: str):
        ...
        self.notification_sent_flags = {
            k: v for k, v in self.notification_sent_flags.items() if v > cutoff_time
        }
    ...
    async def get_expiring_tokens_summary(
        ...
        try:
            query = db.query(BrokerConfig).filter(
                and_(BrokerConfig.access_token_expiry.isnot(None))
            )
            ...
            for config in configs:
                ...
                token_info = {
                    "broker_name": config.broker_name,
                    "user_id": config.user_id,
                    "expires_at": config.access_token_expiry.isoformat(),
            ...
            for config in expired_configs:
                try:
                    ...
                    if success:
                        ...
                        # config.access_token_expiry would be updated with new expiry
                    ...
                    logger.error(
                        f"❌ Token refresh failed for {config.broker_name}: {e}"
                    )
