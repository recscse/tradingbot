# """
# ⚠️ DEPRECATED: Enhanced Market Analytics Service

# This service has been REPLACED by the new Real-Time Market Analytics Engine:
# - services/realtime_market_engine.py
# - router/realtime_analytics_router.py

# The new engine provides:
# - Zero-latency pre-computed analytics (vs 2-second cache)
# - Atomic updates with integer precision
# - Professional-grade heatmaps
# - Comprehensive market analytics

# NEW ENDPOINTS:
# - GET /api/v1/analytics/live-market-data
...
# # DEPRECATION WARNING
# logger.warning("⚠️ DEPRECATED: enhanced_market_analytics.py is deprecated. Use realtime_market_engine.py instead.")
...
#         """🚀 FIXED: Get data from optimized market data service instead of instrument_registry"""
#         import time
...
#         # 🚀 CRITICAL FIX: Use optimized market data service as single source of truth
#         try:
#             from services.optimized_market_data_service import optimized_market_service

#             # Get all live prices from optimized service
#             enriched_data = optimized_market_service.get_all_live_prices()
...
#         except Exception as e:
#             logger.error(f"❌ Error fetching data from optimized service: {e}")
#             # Fallback to instrument registry only if optimized service fails
#             try:
#                 from services.instrument_registry import instrument_registry
#                 fallback_data = instrument_registry.get_enriched_prices()
#                 self._enriched_data_cache = fallback_data or {}
#                 self._enriched_data_cache_time = current_time
#                 return self._enriched_data_cache
...
#                 "advance_decline_analysis": self.get_advance_decline_analysis(),  # 🚀 NEW
...
#         """🚀 FIXED: Use optimized market data service exclusively"""
#         if not force_refresh and not self._should_recalculate("top_movers"):
#             cached_result = self.cache.get("top_movers")
#             if cached_result and cached_result.get("gainers"):
#                 return cached_result

#         try:
#             # 🚀 CRITICAL FIX: Use optimized market data service as primary data source
#             from services.optimized_market_data_service import optimized_market_service

#             # Get pre-calculated gainers and losers (O(1) performance)
#             gainers = optimized_market_service.get_top_gainers(limit)
#             losers = optimized_market_service.get_top_losers(limit)

#             if gainers or losers:
#                 logger.debug(f"⚡ Fast path: Got {len(gainers)} gainers, {len(losers)} losers from optimized service")
#             else:
#                 # Fallback to slow path if optimized service has no data
#                 raise Exception("No data in optimized service")

#         except Exception as e:
#             logger.debug(f"⚠️ Optimized service not available, using fallback: {e}")
...
#             movers_data = self._calculate_top_movers_from_enriched_data(enriched_data, limit)
...
#         """🚀 OPTIMIZED: Ultra-fast sector heatmap using pre-grouped sector data"""
#         if not self._should_recalculate("sector_heatmap"):
#             cached_result = self.cache.get("sector_heatmap")
#             if cached_result and cached_result.get("sectors"):
#                 return cached_result

#         try:
#             # 🚀 NEW: Use optimized market data service (O(sectors) not O(stocks))
#             try:
#                 from services.optimized_market_data_service import optimized_market_service

#                 # Get pre-calculated sector performance (ultra-fast)
#                 sector_performance = optimized_market_service.get_sector_performance()

#                 if sector_performance:
#                     logger.debug(f"⚡ Fast path: Got {len(sector_performance)} sectors from optimized service")
...
#                         # Get top and worst performers for this sector
#                         sector_instruments = optimized_market_service.sector_instruments.get(sector_name, [])
#                         sector_stocks = [
#                             optimized_market_service.instruments[key]
#                             for key in sector_instruments
#                             if key in optimized_market_service.instruments and optimized_market_service.instruments[key].ltp > 0
#                         ]

#                         if not sector_stocks:
#                             continue

#                         top_performer = max(sector_stocks, key=lambda x: x.change_percent)
#                         worst_performer = min(sector_stocks, key=lambda x: x.change_percent)

#                         # Calculate additional metrics
#                         total_volume = sum(stock.volume for stock in sector_stocks)
#                         total_value = sum(stock.value_crores for stock in sector_stocks)

#                         sector_entry = {
#                             "sector": sector_name,
#                             "stocks_count": data["total_stocks"],
#                             "avg_change_percent": round(data["avg_change_percent"], 2),
#                             "advancing": data["advancing"],
#                             "declining": data["declining"],
#                             "unchanged": data["total_stocks"] - data["advancing"] - data["declining"],
#                             "strength_score": round(data["strength_score"], 2),
#                             "total_volume": total_volume,
#                             "total_value_crores": round(total_value, 2),
#                             # Performance categorization
#                             "performance_category": self._categorize_sector_performance(data["avg_change_percent"]),
#                             "momentum": self._calculate_sector_momentum({"strength_score": data["strength_score"]}),
#                             # Top/worst performers
#                             "top_performer": {
#                                 "symbol": top_performer.symbol,
#                                 "name": top_performer.name,
#                                 "change_percent": round(top_performer.change_percent, 2),
#                             },
#                             "worst_performer": {
#                                 "symbol": worst_performer.symbol,
#                                 "name": worst_performer.name,
#                                 "change_percent": round(worst_performer.change_percent, 2),
#                             },
#                             # Advanced metrics
#                             "breadth_ratio": data["advancing"] / (data["advancing"] + data["declining"]) if (data["advancing"] + data["declining"]) > 0 else 0.5,
#                             "participation_rate": (data["advancing"] + data["declining"]) / data["total_stocks"] if data["total_stocks"] > 0 else 0,
#                         }
...
#                 else:
#                     # Fallback to slow path if optimized service has no data
#                     raise Exception("No sector data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for sector heatmap, using fallback: {e}")
...
#                 # Legacy calculation
#                 sectors = self._calculate_sector_heatmap_legacy(enriched_data)

#             # Build summary - works for both optimized and legacy paths
...
#         """🚀 OPTIMIZED: Get comprehensive indices data using optimized service with fallback"""
#         if not self._should_recalculate("indices_data"):
#             return self.cache.get("indices_data", {"indices": [], "summary": {}})

#         try:
#             # 🚀 NEW: Use optimized market data service (O(1) performance)
#             try:
#                 from services.optimized_market_data_service import optimized_market_service

#                 # Get pre-calculated indices data (ultra-fast)
#                 indices_data = optimized_market_service.get_indices_data(20)

#                 if indices_data:
#                     logger.debug(f"⚡ Fast path: Got {len(indices_data)} indices from optimized service")

#                     # Process indices for enhanced analytics format
#                     major_market_indices = []
#                     sector_indices = []
#                     all_indices = []

#                     # Define major indices for categorization
#                     major_index_symbols = {"NIFTY", "BANKNIFTY", "FINNIFTY", "SENSEX", "MIDCPNIFTY"}

#                     for index in indices_data:
#                         # Convert optimized service format to enhanced analytics format
#                         processed_index = {
#                             "symbol": index.get("symbol", ""),
#                             "name": index.get("name", ""),
#                             "instrument_key": index.get("instrument_key", ""),
#                             "exchange": index.get("exchange", "NSE"),
#                             "last_price": index.get("ltp", 0),
#                             "change": index.get("change", 0),
#                             "change_percent": index.get("change_percent", 0),
#                             "high": index.get("high", 0),
#                             "low": index.get("low", 0),
#                             "open": index.get("open_price", 0),
#                             "close": index.get("previous_close", 0),
#                             "volume": index.get("volume", 0),
#                             "last_updated": index.get("last_updated", ""),
#                             "data_source": "optimized_service",
#                         }

#                         all_indices.append(processed_index)

#                         # Categorize indices
#                         if processed_index["symbol"] in major_index_symbols:
#                             major_market_indices.append(processed_index)
#                         else:
#                             sector_indices.append(processed_index)

#                     # Calculate summary statistics
#                     total_indices = len(all_indices)
#                     major_up = len([idx for idx in major_market_indices if idx.get("change_percent", 0) > 0])
#                     major_down = len([idx for idx in major_market_indices if idx.get("change_percent", 0) < 0])
...
#                             "major_unchanged": len(major_market_indices) - major_up - major_down,
#                             "sector_up": len([idx for idx in sector_indices if idx.get("change_percent", 0) > 0]),
#                             "sector_down": len([idx for idx in sector_indices if idx.get("change_percent", 0) < 0]),
#                         },
#                         "timestamp": datetime.now().isoformat(),
#                         "data_source": "optimized_service"
#                     }
...
#                     return result
#                 else:
#                     # Fallback to legacy path if optimized service has no data
#                     raise Exception("No indices data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for indices data, using fallback: {e}")

#                 # FALLBACK: Legacy implementation
#                 return self._legacy_get_indices_data()
...
#     def _legacy_get_indices_data(self) -> Dict[str, Any]:
#         """Legacy indices data implementation for fallback"""
#         try:
#             from services.instrument_registry import instrument_registry
...
#         """🚀 OPTIMIZED: Ultra-fast market sentiment using pre-calculated summary"""
#         if not self._should_recalculate("market_sentiment"):
#             cached_result = self.cache.get("market_sentiment")
#             if cached_result and cached_result.get("sentiment"):
#                 return cached_result
...
#                 # Get pre-calculated market summary (ultra-fast)
#                 market_summary = optimized_market_service.get_market_summary()

#                 if market_summary and "error" not in market_summary:
#                     logger.debug(f"⚡ Fast path: Got market summary from optimized service")
#                 else:
#                     # Fallback to slow path if optimized service has no data
#                     raise Exception("No market data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for market sentiment, using fallback: {e}")
...
#         """🚀 OPTIMIZED: Ultra-fast volume analysis using pre-sorted data"""
#         if not force_refresh and not self._should_recalculate("volume_analysis"):
#             cached_result = self.cache.get("volume_analysis")
#             if cached_result and cached_result.get("volume_leaders"):
#                 return cached_result
...
#                 # Get pre-sorted volume leaders (ultra-fast)
#                 volume_leaders = optimized_market_service.get_volume_leaders(20)
#                 value_leaders = optimized_market_service.get_value_leaders(15)

#                 if volume_leaders:
#                     logger.debug(f"⚡ Fast path: Got {len(volume_leaders)} volume leaders from optimized service")
#                 else:
#                     # Fallback to slow path if optimized service has no data
#                     raise Exception("No data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for volume analysis, using fallback: {e}")
...
#         """🚀 OPTIMIZED: Market breadth analysis using pre-calculated advance/decline data"""
#         try:
#             # 🚀 NEW: Use optimized market data service (O(1) performance)
#             try:
#                 from services.optimized_market_data_service import optimized_market_service

#                 # Get pre-calculated market breadth (ultra-fast)
#                 market_breadth = optimized_market_service.get_market_breadth()

#                 if market_breadth and "error" not in market_breadth:
#                     logger.debug(f"⚡ Fast path: Got market breadth from optimized service")
#                     return market_breadth
#                 else:
#                     # Fallback to slow path if optimized service has no data
#                     raise Exception("No market breadth data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for market breadth, using fallback: {e}")
...
#         """🚀 OPTIMIZED: Ultra-fast intraday opportunities using pre-sorted F&O stocks"""
...
#         try:
#             # 🚀 NEW: Use optimized market data service (O(fno_count) not O(all_stocks))
#             try:
#                 from services.optimized_market_data_service import optimized_market_service

#                 # Get F&O stocks directly (ultra-fast)
#                 fno_stocks = optimized_market_service.get_fno_stocks(100)  # Get top 100 F&O stocks by volume

#                 if fno_stocks:
#                     logger.debug(f"⚡ Fast path: Got {len(fno_stocks)} F&O stocks from optimized service")

#                     # Filter for momentum and volume from pre-sorted F&O stocks
#                     high_momentum = [s for s in fno_stocks if abs(s.get("change_percent", 0)) > 3]
#                     high_volume = [s for s in fno_stocks if s.get("volume", 0) > 1000000]

#                     # F&O-specific filters with lower thresholds (already F&O stocks)
...
#                     # All F&O stocks are already marked as F&O
#                     for stock in fno_stocks:
#                         stock["is_fno"] = True
#                         stock["has_derivatives"] = True

#                 else:
#                     # Fallback to slow path if optimized service has no data
#                     raise Exception("No F&O data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for intraday stocks, using fallback: {e}")
...
#         """🚀 OPTIMIZED: Performance summary statistics using optimized service with fallback"""
#         try:
#             # 🚀 NEW: Use optimized market data service (O(1) performance)
#             try:
#                 from services.optimized_market_data_service import optimized_market_service
...
#                 if market_summary and "error" not in market_summary:
#                     logger.debug("⚡ Fast path: Got performance summary from optimized service")

#                     # Get top gainer and loser for additional metrics
#                     top_gainers = optimized_market_service.get_top_gainers(1)
#                     top_losers = optimized_market_service.get_top_losers(1)

#                     max_gainer = top_gainers[0].get("change_percent", 0) if top_gainers else 0
#                     max_loser = top_losers[0].get("change_percent", 0) if top_losers else 0

#                     # Calculate average change from advancing/declining ratio
#                     total_stocks = market_summary.get("total_stocks", 0)
#                     advancing = market_summary.get("advancing", 0)
#                     declining = market_summary.get("declining", 0)

#                     # Estimate average change based on market breadth
#                     avg_change = market_summary.get("market_breadth_percent", 0) / 2  # Rough estimate

#                     result = {
#                         "total_stocks": total_stocks,
#                         "avg_change": round(avg_change, 2),
#                         "max_gainer": round(max_gainer, 2),
#                         "max_loser": round(max_loser, 2),
#                         "positive_stocks": advancing,
#                         "negative_stocks": declining,
#                         "unchanged_stocks": market_summary.get("unchanged", 0),
#                         "advance_decline_ratio": market_summary.get("advance_decline_ratio", 0),
#                         "market_breadth_percent": market_summary.get("market_breadth_percent", 0),
#                         "total_volume": market_summary.get("total_volume", 0),
#                         "total_value_crores": market_summary.get("total_value_crores", 0),
#                         "timestamp": datetime.now().isoformat(),
#                         "data_source": "optimized_service"
#                     }
...
#                     return result
#                 else:
#                     # Fallback to legacy method if optimized service has no data
#                     raise Exception("No market summary data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for performance summary, using fallback: {e}")

#                 # FALLBACK: Legacy implementation
#                 return self._legacy_get_performance_summary()

#         except Exception as e:
#             logger.error(f"Error getting performance summary: {e}")
#             return {"error": str(e), "timestamp": datetime.now().isoformat()}

#     def _legacy_get_performance_summary(self) -> Dict[str, Any]:
#         """Legacy performance summary implementation for fallback"""
#         try:
#             stocks = self.get_all_live_stocks()

#             if not stocks:
#                 return {"error": "No stocks data available", "data_source": "legacy_fallback"}
...
#                 "max_gainer": round(max(changes), 2) if changes else 0,
#                 "max_loser": round(min(changes), 2) if changes else 0,
...
#             }
#         except Exception as e:
#             logger.error(f"Error in legacy performance summary calculation: {e}")
#             return {"error": str(e), "data_source": "legacy_error"}

#     def get_advance_decline_analysis(self) -> Dict[str, Any]:
#         """🚀 OPTIMIZED: Comprehensive advance/decline analysis using pre-calculated data"""
#         try:
#             # 🚀 NEW: Use optimized market data service (O(1) performance)
#             try:
#                 from services.optimized_market_data_service import optimized_market_service

#                 # Get pre-calculated advance/decline analysis (ultra-fast)
#                 advance_decline_data = optimized_market_service.get_advance_decline_analysis()

#                 if advance_decline_data and "error" not in advance_decline_data:
#                     logger.debug(f"⚡ Fast path: Got advance/decline analysis from optimized service")

#                     # Add additional insights
#                     advance_decline_data["insights"] = self._generate_advance_decline_insights(advance_decline_data)
#                     return advance_decline_data
#                 else:
#                     # Fallback to slow path if optimized service has no data
#                     raise Exception("No advance/decline data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for advance/decline analysis, using fallback: {e}")

#                 # Fallback to legacy market breadth
#                 market_breadth = self.get_market_breadth()
#                 return {
...
#                     "advance_decline_ratio": market_breadth.get("advance_decline_ratio", 0),
#                     "market_breadth_percent": market_breadth.get("breadth_percentage", 0),
#                     "error": "Limited data - using market breadth fallback",
#                     "timestamp": datetime.now().isoformat(),
#                 }

#         except Exception as e:
#             logger.error(f"Error getting advance/decline analysis: {e}")
#             return {"error": str(e), "timestamp": datetime.now().isoformat()}

#     def _generate_advance_decline_insights(self, data: Dict[str, Any]) -> List[str]:
#         """Generate trading insights from advance/decline data"""
#         insights = []

#         try:
#             ratio = data.get("advance_decline_ratio", 0)
#             breadth_pct = data.get("market_breadth_percent", 0)
#             new_highs = data.get("new_highs", 0)
#             new_lows = data.get("new_lows", 0)

#             # Ratio insights
#             if ratio > 3:
#                 insights.append("Very strong market participation - widespread buying interest")
#             elif ratio > 2:
#                 insights.append("Strong bullish momentum - good time for momentum strategies")
#             elif ratio < 0.5:
#                 insights.append("Weak market - consider defensive strategies")

#             # Breadth insights
#             if breadth_pct > 20:
#                 insights.append("Broad-based rally - most stocks participating")
#             elif breadth_pct < -20:
#                 insights.append("Broad-based decline - market stress evident")

#             # New highs/lows insights
#             if new_highs > new_lows * 2:
#                 insights.append("Strong leadership - many stocks breaking to new highs")
#             elif new_lows > new_highs * 2:
#                 insights.append("Market weakness - many stocks making new lows")
...
#         """🚀 OPTIMIZED: Stocks making new highs and lows using pre-calculated data"""
#         try:
#             # 🚀 NEW: Use optimized market data service (O(1) performance)
#             try:
#                 from services.optimized_market_data_service import optimized_market_service

#                 # Get pre-calculated new highs/lows (ultra-fast)
#                 new_highs = optimized_market_service.get_new_highs(20)
#                 new_lows = optimized_market_service.get_new_lows(20)

#                 if new_highs or new_lows:
#                     logger.debug(f"⚡ Fast path: Got {len(new_highs)} new highs, {len(new_lows)} new lows from optimized service")
...
#                             "high_low_ratio": len(new_highs) / len(new_lows) if len(new_lows) > 0 else float('inf'),
#                         },
#                         "timestamp": datetime.now().isoformat(),
#                     }
#                 else:
#                     # Fallback to slow path if optimized service has no data
#                     raise Exception("No new highs/lows data in optimized service")

#             except Exception as e:
#                 logger.debug(f"⚠️ Optimized service not available for record movers, using fallback: {e}")
...
#                 for stock in stocks:
#                     ltp = stock.get("last_price", 0) or stock.get("ltp", 0)
#                     high = stock.get("high", 0)
#                     low = stock.get("low", 0)

#                     if ltp > 0 and high > 0:
#                         # Near day high (simplified)
#                         if (ltp / high) >= 0.99:
...
#     def _calculate_top_movers_from_enriched_data(self, enriched_data: Dict[str, Any], limit: int = 20) -> Dict[str, Any]:
#         """Calculate top movers from enriched data"""
#         try:
#             if not enriched_data:
#                 return {"gainers": [], "losers": []}

#             # Process enriched data into stock list
#             stocks = []
#             for instrument_key, data in enriched_data.items():
#                 if (data.get("ltp") or data.get("last_price")) and data.get("symbol"):
#                     stock_entry = {
...
#         except Exception as e:
#             logger.error(f"Error calculating top movers from enriched data: {e}")
...
#     def _calculate_sector_heatmap_legacy(self, enriched_data: Dict[str, Any]) -> List[Dict[str, Any]]:
#         """Legacy sector heatmap calculation for fallback"""
#         try:
#             sectors_data = {}
...
#                 except Exception as e:
#                     logger.warning(f"⚠️ Error processing stock for legacy sector heatmap: {e}")
#                     continue
...
#                 top_performer = max(data["stocks"], key=lambda x: x.get("change_percent", 0))
#                 worst_performer = min(data["stocks"], key=lambda x: x.get("change_percent", 0))
...
#                     "strength_score": round((data["advancing"] - data["declining"]) / data["total_stocks"] * 100, 2) if data["total_stocks"] > 0 else 0,
#                     "total_volume": data["total_volume"],
#                     "performance_category": self._categorize_sector_performance(avg_change),
#                     "momentum": self._calculate_sector_momentum({"strength_score": (data["advancing"] - data["declining"]) / data["total_stocks"] * 100 if data["total_stocks"] > 0 else 0}),
#                     "top_performer": {
...
#         except Exception as e:
#             logger.error(f"Error in legacy sector heatmap calculation: {e}")
#             return []
